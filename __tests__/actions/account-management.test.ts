...
describe('Account Management Actions', () => {
  ...
  describe('updateProfile', () => {
    it('로그인하지 않은 경우 오류가 반환되어야 함', async () => {
      ...
      // Act
      const result = await updateProfile({ name: 'Test', email: 'test@example.com' });
    ...
    it('이름이 비어있는 경우 오류가 반환되어야 함', async () => {
      ...
      // Act
      const result = await updateProfile({ name: '', email: 'test@example.com' });
    ...
    it('잘못된 이메일 형식인 경우 오류가 반환되어야 함', async () => {
      ...
      // Act
      const result = await updateProfile({ name: 'Test Name', email: 'invalid-email' });

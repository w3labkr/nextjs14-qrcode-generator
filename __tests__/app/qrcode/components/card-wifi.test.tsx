...
const qrcodeFormSchema = z.object({
  ...
  wifiSsid: z.string().min(1, "Wi-Fi 네트워크 이름(SSID)을 입력해주세요.").optional(),
...
function CardWifiWrapper({ defaultValues }: { defaultValues?: Partial<QrcodeFormValues> }) {
...
describe('CardWifi', () => {
  describe('렌더링', () => {
    it('Wi-Fi 네트워크 카드가 올바른 제목과 설명으로 렌더링되어야 함', () => {
      ...
      expect(screen.getByText('QR 코드에 포함할 Wi-Fi 네트워크 정보를 입력하세요.')).toBeInTheDocument();
    });
    ...
    it('모든 Wi-Fi 설정 필드가 렌더링되어야 함', () => {
      ...
      expect(screen.getByPlaceholderText('Wi-Fi 네트워크 이름을 입력하세요.')).toBeInTheDocument();
      
      // 비밀번호 필드 (필수)
      expect(screen.getByText('비밀번호')).toBeInTheDocument();
      expect(screen.getAllByText('(필수)')[1]).toBeInTheDocument();
      expect(screen.getByPlaceholderText('Wi-Fi 비밀번호를 입력하세요.')).toBeInTheDocument();
    ...
    it('비밀번호 필드가 password 타입이어야 함', () => {
      ...
      const passwordInput = screen.getByPlaceholderText('Wi-Fi 비밀번호를 입력하세요.');
    ...
    it('WiFi QR 코드 사용 가이드가 표시되어야 함', () => {
      ...
      expect(screen.getByText('스마트폰 카메라 앱을 열거나 QR 코드 스캐너를 사용하세요')).toBeInTheDocument();
      expect(screen.getByText('QR 코드를 스캔하면 WiFi 연결 알림이 나타납니다')).toBeInTheDocument();
      expect(screen.getByText('알림을 탭하여 자동으로 WiFi에 연결하세요')).toBeInTheDocument();
      expect(screen.getByText('연결이 되지 않으면 WiFi 설정에서 수동으로 확인해보세요')).toBeInTheDocument();
      expect(screen.getByText('일부 구형 기기는 WiFi QR 코드를 지원하지 않을 수 있습니다.')).toBeInTheDocument();
    });
  });
  ...
  describe('사용자 입력', () => {
    it('네트워크 이름(SSID) 입력이 정상적으로 동작해야 함', () => {
      ...
      const ssidInput = screen.getByPlaceholderText('Wi-Fi 네트워크 이름을 입력하세요.');
  ...
  describe('사용 사례', () => {
    it('일반 가정용 WiFi 설정을 입력할 수 있어야 함', () => {
      ...
      fireEvent.change(passwordInput, { target: { value: 'familyPassword2024' } });

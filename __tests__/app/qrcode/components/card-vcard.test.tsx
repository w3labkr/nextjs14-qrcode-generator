...
describe("CardVCard", () => {
  ...
  describe("사용자 입력", () => {
    ...
    it("웹사이트 입력이 정상적으로 동작해야 함", () => {
      ...
      fireEvent.change(websiteInput, {
        target: { value: "https://mycompany.com" },
      });
  ...
  describe("기본값", () => {
    it("기본값이 설정되어 있으면 모든 필드에 표시되어야 함", () => {
      ...
      expect(
        screen.getByDisplayValue("https://xyz-corp.com"),
      ).toBeInTheDocument();
      expect(
        screen.getByDisplayValue("인천광역시 연수구 송도대로 123"),
      ).toBeInTheDocument();
    });
  ...
  describe("레이아웃", () => {
    ...
    it("웹사이트와 주소 필드가 전체 너비를 차지해야 함", () => {
      ...
      expect(
        websiteInput.closest(".col-span-1.md\\:col-span-2"),
      ).toBeInTheDocument();
      expect(
        addressInput.closest(".col-span-1.md\\:col-span-2"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("통합 테스트", () => {
    it("완전한 명함 정보를 입력할 수 있어야 함", () => {
      ...
      fireEvent.change(jobTitleInput, {
        target: { value: "시니어 풀스택 개발자" },
      });
      fireEvent.change(websiteInput, {
        target: { value: "https://kimdev.tech" },
      });
      fireEvent.change(addressInput, {

...
-- 4. User 접근 정책 생성 (사용자는 자신의 정보만 접근 가능)
CREATE POLICY users_self_policy ON users
  USING (id = current_setting('app.current_user_id')::text);

-- 5. Account 테이블 RLS 활성화
ALTER TABLE accounts ENABLE ROW LEVEL SECURITY;

-- 6. Account 접근 정책 생성 (사용자는 자신의 계정만 접근 가능)
CREATE POLICY accounts_user_policy ON accounts
  USING (user_id = current_setting('app.current_user_id')::text);

-- 7. Session 테이블 RLS 활성화
ALTER TABLE sessions ENABLE ROW LEVEL SECURITY;

-- 8. Session 접근 정책 생성 (사용자는 자신의 세션만 접근 가능)
CREATE POLICY sessions_user_policy ON sessions
  USING (user_id = current_setting('app.current_user_id')::text);

-- 9. ApplicationLog 테이블 RLS 활성화 (통합 로그 시스템)
ALTER TABLE application_logs ENABLE ROW LEVEL SECURITY;

-- 10. ApplicationLog 일반 사용자 접근 정책
